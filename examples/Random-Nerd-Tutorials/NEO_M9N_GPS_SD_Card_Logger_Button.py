...
gps_serial = UART(1, baudrate=38400, tx=Pin(4), rx=Pin(5))
...
def mount_sdcard(spi, cs_pin):
    try:
        ...
        print('An error occured mounting the SD card:', e)
...
def convert_to_decimel_degrees(coord):
    degrees = coord[0]
    minutes = coord[1]
    hemisphere = coord[2]
            ...
            for byte in data:
                ...
                if stat is not None:
                    ...
                    if ticks_diff(current_time, last_print_time) >= 3000:
                        ...
                        longitude = convert_to_decimel_degrees(my_gps.longitude)
                        latitude = convert_to_decimel_degrees(my_gps.latitude)
